...
test('formatDurationSec keeps sub-hour format', () => {
  assert.equal(formatDurationSec(45), '45s');
  assert.equal(formatDurationSec(60, { short: true }), '1m');
  assert.equal(formatDurationSec(90, { short: true }), '1m 30s');
  assert.equal(formatDurationSec(90), '1m 30s');
});

test('formatDurationSec shows hours above 60 minutes', () => {
  assert.equal(formatDurationSec(3600, { short: true }), '1h');
  assert.equal(formatDurationSec(3665, { short: true }), '1h 1m 5s');
  assert.equal(formatDurationSec(82772, { short: true }), '22h 59m 32s');
});
